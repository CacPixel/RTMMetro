...
//    }
    ...
    {
        return isDiagonal ? NGTMath.floor(NGTMath.normalizeAngle((double) placer.rotationYaw + 180.0D) / 90.0D) & 3 :
                NGTMath.floor(NGTMath.normalizeAngle((double) placer.rotationYaw + 180.0D) / 90.0D + 0.5D) & 3;
    }
    ...
    protected boolean onBlockActivated(BlockArgHolder holder, float hitX, float hitY, float hitZ)
    {
        ...
        if (itemstack.getItem() == Item.getItemFromBlock(RTMMetroBlock.MARKER_ADVANCED)
                ...
                || itemstack.getItem() == RTMMetroItems.railEditor)
        {
            if (world.isRemote)
            {
...
//        if (itemstack.getItem() == RTMMetroItems.railAdvanced || itemstack.getItem() == RTMItem.itemLargeRail) {
        if (!world.isRemote && this.onMarkerActivated(world, i, j, k, entityplayer, true) &&
        ...
        for (int i1 = 0; i1 < j; ++i1)
        {
            for (int j1 = 0; j1 < l; ++j1)
            {
                for (int k1 = 0; k1 < j; ++k1)
                {
                    ...
                    int yy = y - k + j1;
                    int zz = z - i + k1;
                    RailPosition railposition = this.getRailPosition(world, xx, yy, zz);
...
//            NGTLog.debug("getAllMarkers (cacpixel method) time:%.2f us", duration / 1000.0f);
    ...
    public static void modernMarkerSortMethod(List<RailPosition> list, EntityPlayer player)
    {
        list.sort((arg0, arg1) -> {
            ...
            else if (player != null)
            {
                ...
                double l0 = Math.sqrt(
                        Math.pow(arg0.blockX - px, 2) + Math.pow(arg0.blockY - py, 2) + Math.pow(arg0.blockZ - pz, 2));
                double l1 = Math.sqrt(
                        Math.pow(arg1.blockX - px, 2) + Math.pow(arg1.blockY - py, 2) + Math.pow(arg1.blockZ - pz, 2));
    ...
    public static boolean createRail(World world, int x, int y, int z, List<RailPosition> rps, ResourceStateRail state,
                                     boolean makeRail, boolean isCreative)
    {
        if (rps.size() == 1)
        {
...
//            }
    ...
    private static void prepareNormalRail(World world, RailPosition start, RailPosition end, ResourceStateRail prop,
                                          boolean makeRail, boolean isCreative)
    {
        ...
        if (makeRail && originalRailMap.canPlaceRail(world, isCreative, prop))
        {
            ...
            if (!world.isRemote)
            {
                MarkerManager.sendPacket((TileEntityMarkerAdvanced) world.getTileEntity(
                        new BlockPos(start.blockX, start.blockY, start.blockZ)), true);
            }
...
//                split = Math.max(1, marker.splits);
    ...
    private static void createNormalRail(World world, RailPosition start, RailPosition end, ResourceStateRail prop,
                                         boolean makeRail, boolean isCreative)
    {
...
//    private static boolean createCustomRail(World world, RailPosition rp, ResourceStateRail prop, boolean makeRail,
//    boolean isCreative) {
...
//            // 是否应该遍历所有的RailPositions？
//            if (!world.isRemote)
//                MarkerManager.sendPacket((TileEntityMarkerAdvanced) world.getTileEntity(new BlockPos(rps.get(0)
//                .blockX, rps.get(0).blockY, rps.get(0).blockZ)), true);
//
...
//    }
    ...
    private static boolean createSwitchRail(World world, int x, int y, int z, List<RailPosition> list,
                                            ResourceStateRail prop, boolean makeRail, boolean isCreative)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (makeRail && !flag)
                {
                    ...
                    if (!world.isRemote)
                        MarkerManager.sendPacket((TileEntityMarkerAdvanced) world.getTileEntity(
                                new BlockPos(railposition.blockX, railposition.blockY, railposition.blockZ)), true);
                    ...
                    {
                        BlockUtil.setBlock(world, railposition1.blockX, railposition1.blockY, railposition1.blockZ,
                                RTMMetroBlock.LARGE_RAIL_SWITCH_BASE_ADVANCED, 0, 3);
                        TileEntityLargeRailSwitchBase tileentitylargerailswitchbase =
                                (TileEntityLargeRailSwitchBase) BlockUtil.getTileEntity(
                                        world, railposition1.blockX, railposition1.blockY, railposition1.blockZ);
                    ...
                    TileEntityLargeRailSwitchCoreAdvanced TileEntityLargeRailSwitchCoreAdvanced =
                            (TileEntityLargeRailSwitchCoreAdvanced) BlockUtil.getTileEntity(
                                    world, x, y, z);
...
//    private static boolean createTurntable(World world, RailPosition start, RailPosition end, ResourceStateRail
//    prop, boolean makeRail, boolean isCreative) {
//        int i = 0;
//        int j = start.blockY;
//        int k = 0;
//        int l = 0;
...
//                TileEntityTurnTableCore tileentityturntablecore = (TileEntityTurnTableCore) BlockUtil.getTileEntity
//                (world, i, j, k);
...
//    }
    ...
    {
        TileEntity tileentity = BlockUtil.getTileEntity(world, x, y, z);
        return tileentity instanceof TileEntityMarkerAdvanced ? ((TileEntityMarkerAdvanced) tileentity).getMarkerRP() :
                null;
    }

    private TileEntityMarkerAdvanced getTileEntity(World world, int x, int y, int z)
    {
        TileEntity tileentity = BlockUtil.getTileEntity(world, x, y, z);
