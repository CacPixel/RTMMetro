...
public class AdvancedRailRenderer extends TileEntityPartsRenderer<ModelSetRigidCatenary>
{
    protected int currentRailIndex;
    private final FloatBuffer convBuf = FloatBuffer.wrap(
            new float[]{1.0F, 0.0F, 0.0F, 0.0F, 0.0F, 1.0F, 0.0F, 0.0F, 0.0F, 0.0F, 1.0F, 0.0F, 0.0F, 0.0F, 0.0F,
                    1.0F});

    public AdvancedRailRenderer(String... par1)
    {
        super(par1);
    }
...
//        this.renderRailDynamic(tileEntity, par2, par4, par6, par8);
        ...
        else
        {
            ...
            for (int var7 = 0; var7 < var6; ++var7)
            {
                ...
                for (int i = 0; i <= max; ++i)
                {
                    ...
                    float[] array = new float[]{moveX + (float) (curPoint[1] - stPoint[1]),
                            (float) (rm.getRailHeight(max, i) - startH), moveZ + (float) (curPoint[0] - stPoint[0]),
                            rm.getRailRotation(max, i), -rm.getRailPitch(max, i), rm.getCant(max, i)};
                    list.add(array);
                }
            }
                                    ...
                                    int endIndex, Parts... pArray)
    {
        ...
        for (int i = startIndex; i <= endIndex; ++i)
        {
            ...
            this.setBrightness(this.getBrightness(tileEntity.getWorld(), NGTMath.floor(origPos[1] + (double) x0),
                    tileEntity.getPos().getY(), NGTMath.floor(origPos[0] + (double) z0)));
            GL11.glPushMatrix();
